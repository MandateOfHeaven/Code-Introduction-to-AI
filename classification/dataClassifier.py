...
def runClassifier(args, options):
    ...
    if options.data == 'digits':
        ...
        from sentimentAnalysis import loadTextData
        rawTrainingData, rawTrainingLabels, rawValidationData, rawValidationLabels, rawTestData, rawTestLabels = loadTextData()
    ...
    if options.visualize and options.data=='digits':
        digitImg = np.array([rawTrainingData[np.argwhere(rawTrainingLabels==i)[0,0]] for i in range(10)])
        featureExtractor.visualize(digitImg)
