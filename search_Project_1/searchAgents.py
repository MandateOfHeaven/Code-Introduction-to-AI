            ...
            if not self.walls[nextx][nexty]:
                ...
                for i in range(0, 4):
                    if nextPosition == self.corners[i]:
                        TempState[i] = True
                NewState = (nextPosition, tuple(TempState))
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0, 4):
        if state[1][i]:
            ...
            ToThisCorner = abs(
                ThisCorner[0] - CurrentPosition[0]) + abs(ThisCorner[1] - CurrentPosition[1])
            Distance.append(ToThisCorner)
    return min(Distance)
...
def foodHeuristic(state, problem):
    ...
    def manhhatandistance(a, b):
        return abs(a[0] - b[0]) + abs(a[1] - b[1])
    for food in foodList:
        # DistanceToFood = mazeDistance(
        #     position, food, problem.startingGameState)
        DistanceToFood = manhhatandistance(position, food)
        DistanceList.append(DistanceToFood)
        # Distance = sum(DistanceList) / max(len(DistanceList), 1)
